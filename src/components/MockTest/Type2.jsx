...
export default function Type2({ questionDetails, callbackForType2 }) {
  ...
  // retreives the selected answer if question is attemtped
  const selectedAnswers = questionDetails?.question_selected_mock[0]
    ?.correct_option_json
    ? JSON.parse(
        questionDetails?.question_selected_mock[0]?.correct_option_json
      )
    : [];
